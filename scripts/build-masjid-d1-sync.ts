...
function buildSql(rows: CanonicalMasjidRow[], batchSize: number, wrapTransaction: boolean): string {
  ...
  const sourceSystems = [...new Set(rows.map((row) => row.sourceSystem))];
  const statements = [
  ...
  if (sourceSystems.length > 0) {
    statements.push(
      `DELETE FROM masjids WHERE source_system IN (${sourceSystems.map((sourceSystem) => escapeSqlString(sourceSystem)).join(", ")});`,
    );
  } else if (sourceVersions.length > 0) {
    statements.push(
