    ...
    return {
      ...
      sourceClass:
        typeof row.sourceClass === "string" && row.sourceClass.trim().length > 0
          ...
          : null,
      sourceType:
        typeof row.sourceType === "string" && row.sourceType.trim().length > 0 ? row.sourceType.trim() : null,
      sourceCategory:
...
function buildSql(rows: CanonicalMasjidRow[], batchSize: number): string {
...
async function main() {
  ...
  await Bun.write(output, buildSql(rows, options.batchSize));
