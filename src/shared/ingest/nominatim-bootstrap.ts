...
export function validateStructuredExportShape(input: unknown): StructuredExportShape {
  ...
  const items: StructuredExportItem[] = rawItems.map((item, index) => {
    ...
    return {
      osm_type:
        typeof row.osm_type === "string" && row.osm_type.trim().length > 0
          ? row.osm_type.trim()
          : undefined,
      osm_id:
        typeof row.osm_id === "string" || typeof row.osm_id === "number"
          ? row.osm_id
          : undefined,
